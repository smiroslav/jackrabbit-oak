...
package org.apache.jackrabbit.oak.store.remote.io;
...
import org.apache.jackrabbit.oak.store.remote.store.Type;
import org.apache.jackrabbit.oak.store.remote.store.Value;

public abstract class KryoConverter {

    protected abstract ID readID(Input input);

    protected abstract void writeID(Output output, ID id);

    protected abstract Node createNode(ID properties, ID children);

    public final Map<String, Value> readProperties(byte[] bytes) {
    ...
    public final byte[] writeProperties(Map<String, Value> properties) {
    ...
    public final Map<String, ID> readChildren(byte[] bytes) {
        try (Input input = new Input(bytes)) {
    ...
    public final byte[] writeChildren(Map<String, ID> children) {
        ByteArrayOutputStream stream = new ByteArrayOutputStream();
        try (Output output = new Output(stream)) {
            output.writeVarInt(children.size(), true);
            for (Entry<String, ID> e : children.entrySet()) {
                output.writeString(e.getKey());
                writeID(output, e.getValue());
            }
        }
        return stream.toByteArray();
    }

    public final Node readNode(byte[] bytes) {
        try (Input input = new Input(bytes)) {
            ID properties = readID(input);
            ID children = readID(input);
            return createNode(properties, children);
        }
    }

    public final byte[] writeNode(ID properties, ID children) {
        ByteArrayOutputStream stream = new ByteArrayOutputStream();
        try (Output output = new Output(stream)) {
            writeID(output, properties);
            writeID(output, children);
        }
        return stream.toByteArray();
    }

    public final ID readID(byte[] bytes) {
        try (Input input = new Input(bytes)) {
            return readID(input);
        }
    }

    public final byte[] writeID(ID id) {
        ByteArrayOutputStream stream = new ByteArrayOutputStream();
        try (Output output = new Output(stream)) {
            writeID(output, id);
        }
    ...
    private static Value readValue(Input input) {
        Type type = Type.values()[input.readVarInt(true)];
